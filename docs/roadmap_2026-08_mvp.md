...
| # | Item | Owner | Acceptance |
|---|---|---|---|
| B1 | RPM → m/s calibration with measured uncertainty | user (operator) | speed model fitted, residual stated, `--rpm-speed-model` wired into the corridor evaluator. Procedure: A6. **UNBLOCKED 2026-08-17:** independent phone video measured firmware/true ratios of 0.991/0.995 on IMG_2536, validating both PPR constants. The earlier ~23/31% claims extrapolated a nonlinear 300-RPM point across the whole map. A 500 command now plateaus at L=456/R=502 inside the existing arm predicate, and video measured 700–1000 within about 3% of command. Keep `control_14`; no map refit or `control_15` is a prerequisite. The first live 500-RPM pass must still satisfy every fresh/stable/spread gate in protocol §5.** |
