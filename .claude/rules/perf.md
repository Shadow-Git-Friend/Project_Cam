...
- Scale everything through `sc(H, s)` (normalised to 720) so the board is resolution-independent, and derive layout offsets from `cv2.getTextSize` rather than magic pixel constants — a hardcoded offset collided `1400 mm` with `stabilise 0.6 s` at one width.
