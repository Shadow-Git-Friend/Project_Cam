...
# exec: the caller's $! must be the viewer itself, not this shell. The training
# wrapper signals that PID directly (SIGINT for a clean MP4/exit) and watches it
# to decide when the session is over.
exec ./venv/bin/python Parallel_working/scripts/live_4cam_arena_view_parallel.py \
