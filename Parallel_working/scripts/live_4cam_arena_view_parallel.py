...
def main():
    ...
    ap.add_argument("--window-pane", choices=["none", "left", "right", "full"],
                    default="none",
                    help="Place the 3D arena window on one half of the desktop work "
                         "area so a second window (e.g. the training drill board) "
                         "fits beside it without dragging. Display-only: window "
                         "geometry only, no change to the render, triangulation or "
                         "UDP. --fullscreen overrides it.")
    ...
    arena_pane = "none" if args.fullscreen else args.window_pane
    if use_cv2_viz and args.show_3d:
        # A requested pane means the window is being tiled next to another one
        # (drill board / projector board), so drop the Qt toolbar: with chrome
        # present `resizeWindow` sizes the frame instead of the image and the
        # pane comes out short. Untiled runs keep the historical window flags.
        arena_flags = cv2.WINDOW_NORMAL
        if arena_pane != "none":
            arena_flags |= cv2.WINDOW_GUI_NORMAL
        cv2.namedWindow("Live 3D Arena", arena_flags)
        if args.fullscreen:
            cv2.setWindowProperty("Live 3D Arena", cv2.WND_PROP_FULLSCREEN,
                                  cv2.WINDOW_FULLSCREEN)
    arena_window_placed = arena_pane == "none" or not (use_cv2_viz and args.show_3d)

    def place_arena_window(frame_w, frame_h):
        """Tile the 3D window once its first frame has sized it (display-only)."""
        try:
            from project_cam.viz.window import pane_rect, place_window  # noqa: E402
            rect = pane_rect(arena_pane, aspect=frame_w / max(1, frame_h))
            place_window("Live 3D Arena", rect, pump=cv2.waitKey)
            if rect is not None:
                print(f"[INFO] 3D window placed on the {arena_pane} pane: {rect}")
        except Exception as exc:  # placement is cosmetic — never fail the run
            print(f"[WARN] 3D window placement skipped: {exc}")
        ...
        while not _stop_flag["v"]:
            ...
            if args.show_3d and (frame_idx % args.viz_every == 0):
                ...
                if use_cv2_viz:
                    ...
                    if not arena_window_placed:
                        arena_window_placed = True
                        place_arena_window(viz_img.shape[1], viz_img.shape[0])
