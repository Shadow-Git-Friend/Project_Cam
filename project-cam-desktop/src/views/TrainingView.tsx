...
import { drillRequest, supportsProjector } from "../launch";
...
export default function TrainingView({
  ...
  // Off by default: a fullscreen board hides the 3D arena view, and the two are
  // tiled side by side on a single-monitor rig. Turn it on for a real projector.
  ...
  const startDrill = () => {
    ...
    run({
      ...
      drill: drillRequest(drill.id, rounds, flip, projector),
      ...
      <div className="flex-1 min-w-0 flex flex-col gap-2 min-h-0">
        ...
        <div className="flex-[3] min-h-0 overflow-y-auto bg-arena-panel border border-arena-yellow/25 rounded-xl p-[18px] flex flex-col gap-3.5">
          ...
          <div className="flex items-center gap-4 pt-1">
            ...
            {supportsProjector(drill.id) && (
              <button
                onClick={() => setProjector(!projector)}
                disabled={running}
                title="Open the drill board fullscreen for a projector. Off keeps it tiled beside the 3D arena view on this screen."
                className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border text-[11px] font-bold tracking-wide ${
                  projector
                    ? "border-arena-yellow/60 text-arena-yellow bg-arena-yellow/[0.08]"
                    : "border-white/[0.14] text-white/50 hover:text-white/80"
          ...
          <p className="text-[10.5px] text-white/35 leading-snug">
            The drill board and the 3D arena view open together, tiled on the two halves of the screen —
            the board waits for the cameras so you never watch an empty scoreboard. Press{" "}
            <span className="font-mono text-white/55">F</span> on the board for fullscreen and again to
            return to its half. Results log to{" "}
            <span className="font-mono text-white/55">output/training_logs/</span>. View-only: the drills
            read the pose stream and never actuate the launcher. Stop anytime with STOP below.
          </p>
        </div>
