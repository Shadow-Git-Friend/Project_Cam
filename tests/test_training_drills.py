...
def test_gk_updown_counts_reps_and_recovery_times():
    ...
    # rep 1: down at t=2.0, back over up-threshold at 3.0, held 0.2s
    d.update(2.0, person(hip_z=400))         # below 550 -> down
    assert d.phase == "down"
    ...
    d.update(4.5, person(hip_z=900))
    d.update(4.55, person(hip_z=800))        # dropped before hold satisfied
    d.update(4.7, person(hip_z=900))
    d.update(4.95, person(hip_z=920))        # held from 4.7 -> rep 2
    assert d.reps == 2
