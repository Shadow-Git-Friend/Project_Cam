...
class GkSaveDrill:
    ...
    def _wrist_in_target(self, joints):
        side, high = self.target
    ...
    def per_corner(self):
        ...
        for high in (True, False):
            for side in (0, 2):
                ...
                rts = [r["reaction_s"] for r in rows if r["reaction_s"] is not None]
                out[name] = {
                    "rounds": len(rows),
                    "saves": len(rts),
    ...
    def summary(self):
        rts = [r["reaction_s"] for r in self.results if r["reaction_s"] is not None]
    ...
    def update(self, now, joints=None):
        ...
        if self.state == "countdown":
            ...
            if z is not None and self.stand_z is not None:
                if self.phase == "up" and z < self.down_thresh:
                    self.phase = "down"
                    self._t_down = now
                    self._up_since = None
    ...
    def update(self, now, joints=None):
        ...
        if self.state == "set_wait":
            ...
            if observed_zone is None:
                ...
                if observed_zone == self.target and elapsed <= self.cue_timeout_s:
...
class CmjDrill:
    ...
    def rises(self):
        return [r["pelvis_rise_mm"] for r in self.results
                if r["pelvis_rise_mm"] is not None]
