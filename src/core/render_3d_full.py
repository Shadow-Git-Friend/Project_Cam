...
OUTPUT_DIR = PROJECT_ROOT / "output" / "frames" / "render_output_full"
...
def main():
    ...
    if valid_frames_count > 0:
        ...
        # Fallback: Rotate -Y to +Z (typical camera setup)
        # Old Y (Down) -> New Z (Up)
        print("[WARN] No skeleton found for alignment. Using default rotation.")
        # Rotation X-axis -90 deg?
        # Cam: X, Y(down), Z(fwd)
        # Plot: X, Y(fwd), Z(up)
        # Map: x->x, y->-z, z->y
    ...
    print(f"Rendering {len(data)} frames with REAL BALL SIZE & JOINTS...")
    ...
    for i in range(len(data)):
        ...
        # Draw Current Ball (Real Size 3D Sphere)
        bx, by, bz = traj_ball_rot[i]
        if not np.isnan(bx):
            # Shift sphere to ball position
            ax.plot_wireframe(sphere_x + bx, sphere_y + by, sphere_z + bz, 
                            color='yellow', alpha=0.6, linewidth=1)
        ...
        if len(sx) > 0:
            ax.scatter(sx, sy, sz, c='white', s=20)

        # Highlight Wrists (9, 10) & Ankles/Feet (15, 16)
        # Red for Wrists, Green for Feet
        for j_idx, col in [(9, 'red'), (10, 'red'), (15, 'lime'), (16, 'lime')]:
            if valid_joints[j_idx]:
                pt = current_skel[j_idx]
                ax.scatter([pt[0]], [pt[1]], [pt[2]], c=col, s=60, edgecolors='white', zorder=10)
    ...
    # Video
    print("\nEncoding video...")
    out_vid = PROJECT_ROOT / "output" / "videos" / "final_3d_full_realball_1.mp4"
