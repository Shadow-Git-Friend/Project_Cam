...
def load_trials_csv(path):
    ...
    with open(path, "r", newline="") as f:
        ...
        for r in reader:
            rows.append(
                {
                    "trial_id": r["trial_id"].strip(),
                    "gt_x_mm": safe_float(r["x_mm"]),
                    "gt_y_mm": safe_float(r["y_mm"]),
                    "gt_z_mm": safe_float(r["z_mm"]),
                }
            )
    return rows
...
def analyze_trial(frames, gt, start_frac, end_frac):
    ...
    for fr in win:
        ...
        if not np.isfinite(arr).all():
            continue
        pts.append(arr)
        cams = fr.get("ball_cams")
        if isinstance(cams, list):
            cams_used.append(float(len(cams)))
        rp = fr.get("ball_reproj_px")
        if rp is not None and np.isfinite(rp):
            reproj.append(float(rp))

    n_win = len(win)
    n_valid = len(pts)
    detect_ratio = (n_valid / n_win) if n_win > 0 else 0.0
    cams_mean = float(np.mean(cams_used)) if len(cams_used) > 0 else np.nan
    reproj_mean = float(np.mean(reproj)) if len(reproj) > 0 else np.nan
...
def main():
    ap = argparse.ArgumentParser(description="Evaluate static ball GT trials and generate error/bias report.")
    ap.add_argument("--trials-csv", required=True, help="CSV with columns: trial_id,x_mm,y_mm,z_mm")
    ap.add_argument("--results-dir", required=True, help="Directory with per-trial JSONs, e.g. B001.json")
        ...
        rows.append(
            {
                ...
                "n_valid_ball_frames": s["n_valid"],
                "detect_ratio_window": s["detect_ratio"],
                "mean_cams_used": s["cams_mean"],
                "mean_ball_reproj_px": s["reproj_mean"],
                "status": "ok" if np.isfinite(s["err_norm"]) else "no_ball_detected",
            }
        )
    ...
    cams_vals = [r["mean_cams_used"] for r in ok_rows if np.isfinite(r["mean_cams_used"])]
    reproj_vals = [r["mean_ball_reproj_px"] for r in ok_rows if np.isfinite(r["mean_ball_reproj_px"])]
    ...
    summary = {
        ...
        "detection": {
            ...
            "mean_cams_used": float(np.mean(cams_vals)) if len(cams_vals) > 0 else np.nan,
            "mean_reprojection_px": float(np.mean(reproj_vals)) if len(reproj_vals) > 0 else np.nan,
        },
    }
    ...
    # Simple correction model: global bias + per-axis linear map.
    ...
    lines = []
    lines.append("# Ball Static GT Error Report")
    lines.append("")
    ...
    lines.append(f"- Mean cameras used: `{summary['detection']['mean_cams_used']:.2f}`")
    lines.append(f"- Mean reprojection error (px): `{summary['detection']['mean_reprojection_px']:.2f}`")
    lines.append("")
