...
def _within_rpm_spread(rpms, vs):
    """Pooled shot-to-shot spread about each RPM's own mean, in m/s.

    Returns 0.0 when every RPM has a single shot — which is itself information:
    a one-shot-per-RPM model has no measured repeatability at all.
    """
    residuals = []
    for rpm in np.unique(rpms):
        group = vs[rpms == rpm]
        residuals.extend(group - group.mean())
    if not residuals:
        return 0.0
...
def fit(points, g, height_m, kind):
    ...
    out = {
        ...
        # How many shots the model rests on, in EVERY branch. A speed quoted
        # without its sample count and residual cannot inform a clearance
        # margin, and the corridor evaluator is downstream of this file.
        "n_shots": int(len(vs)),
    }
    ...
    if kind == "interp" or len(points) < 2:
        ...
        # An interpolating model passes exactly through its points, so a fit
        # residual would be 0 by construction and would read as certainty. The
        # honest uncertainty is the shot-to-shot spread WITHIN each RPM, pooled.
        out["fit_rmse_mps"] = _within_rpm_spread(rpms, vs)
        return out
